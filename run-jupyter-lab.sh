...
if [ "$1" = "--develop" ] ; then
    ...
    sed -i "" -e "s|$PRODUCTION_ENDPOINT|$DEVELOPMENT_ENDPOINT|gw changelog.txt" "$SOURCE_FILE"
    ...
    sed -i "" -e "s|$DEVELOPMENT_ENDPOINT|$PRODUCTION_ENDPOINT|gw changelog.txt" "$SOURCE_FILE"
    if [ -s changelog.txt ]; then
        cat changelog.txt
    fi
